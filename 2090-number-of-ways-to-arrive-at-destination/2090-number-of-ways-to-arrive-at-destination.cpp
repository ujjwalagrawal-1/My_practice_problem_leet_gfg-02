...
typedef vector<vector<ll>> dvecl;
typedef vector<vector<int>> dvec;
typedef vector<pair<ll, ll>> vecpll;
typedef vector<pair<ll, pair<ll, bool>>> vecpllb;
typedef vector<pair<int, pair<int, bool>>> vecpiib;
typedef queue<ll> ql;
...
template <> void _print(bool t) { cerr << (t ? "true" : "false"); }
...
template <typename T> void _print(multiset<T> ms) {
    cerr << "{ ";
    for (T i : ms) {
        _print(i);
        cerr << " ";
...
void _print(const std::vector<std::pair<T1, T2>>& vec) {
    std::cerr << "[";
...
public:
    int countPaths(int n, vector<vector<int>>& roads) {
        ...
        vector<vector<ll>> dis(n, {LLONG_MAX, 0});
        ...
        while (!pq.empty()) {
            ...
            for (auto [neighbor, weight] : adj[node]) {
                if (w + weight < dis[neighbor][0]) {
                    dis[neighbor][0] = (w + weight);
                    dis[neighbor][1] = dis[node][1];
                    pq.push({w + weight, neighbor});
                } else if (w + weight == dis[neighbor][0]) {
                    dis[neighbor][1] = (dis[neighbor][1] + dis[node][1])%M;
                }
            }
        }
        return dis[n - 1][1]%M;
