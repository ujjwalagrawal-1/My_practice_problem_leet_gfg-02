...
class Solution {
public:
int f(dvec &matrix,int x,int y,dvec &dp){
    if(dp[x][y] != -1){
        return dp[x][y];
    }
    int disx[4] = {1,0,-1,0};
    ...
    for(int i = 0;i<4;i++){
        int nx = x+disx[i];
        int ny = y+disy[i];
        if(nx < 0 || ny >= matrix[0].size() || nx >= matrix.size() || ny < 0){
        }
        else{
            if(matrix[nx][ny] > matrix[x][y])
            ans = max(ans,f(matrix,nx,ny,dp)+1);
        }
    }
    return dp[x][y] = ans;
}
    int longestIncreasingPath(vector<vector<int>>& matrix) {
        int ans = 1;
        dvec dp(matrix.size()+1,vec(matrix[0].size()+1,-1));
