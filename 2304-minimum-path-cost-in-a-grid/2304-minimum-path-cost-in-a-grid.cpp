...
class Solution {
  ...
  int getMinPathCost( int sr ,  int sc ,  int er ,  int ec , vector<vector<int>> &grid , vector<vector<int>> &moveCost)
 {
     if(sr == er)
     return grid[sr][sc];
      ...
      for( int cjmp=0;cjmp<=ec;cjmp++)
      {
          if(sr+1 > er)
          break;
          
          mincost = min(getMinPathCost(sr+1,cjmp,er,ec,grid,moveCost)+moveCost[grid[sr][sc]][cjmp] + grid[sr][sc] , mincost);
      }
...
public:
       int minPathCost(vector<vector<int>>& grid, vector<vector<int>>& moveCost) {
         ...
         int sr = 0 , er = grid.size() - 1;
         int sc = 0 , ec = grid[0].size() - 1;

        
        while(sc <= ec)
        {
            mincost = min(getMinPathCost(sr,sc,er,ec,grid,moveCost),mincost);
            sc++;
        }
